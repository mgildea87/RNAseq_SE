...
rule all:
	input:
		...
		expand('fastqc_post_trim/{sample_file}_trimmed{read}_fastqc.html', sample_file = sample_ids, read = read)
...
#		'logs/alignment_reports/{sample}.log'
	...
	shell:
		'STAR {params} --genomeDir %s --runThreadN {threads} --readFilesIn {input.R1} --outFileNamePrefix alignment/{wildcards.sample}_ | samtools view -bh > alignment/{wildcards.sample}.bam' % (genome)
rule count:
	input:
